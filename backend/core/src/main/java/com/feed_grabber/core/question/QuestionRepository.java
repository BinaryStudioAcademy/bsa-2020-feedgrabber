...
public interface QuestionRepository extends JpaRepository<Question, UUID> {
    @Query("SELECT q FROM Question q INNER JOIN q.questionnaires que " +
            "WHERE que.id.questionnaireId = :questionnaireId order by que.index ASC")
    ...
    @Query("SELECT case when (COUNT(q) > 0) then true else false end " +
            ...
            "WHERE q.text = :text AND que.id.questionnaireId = :questionnaireId AND q.category.id = :categoryId")
    ...
    @Query("SELECT case when (COUNT(q) > 0) then true else false end " +
            "FROM Question q INNER JOIN q.questionnaires que " +
            "WHERE q.text = :text AND que.id.questionnaireId = :questionnaireId AND " +
